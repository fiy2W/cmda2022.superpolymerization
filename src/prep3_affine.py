...
for mov_T1_name in mov_T1_names:
    ...
    cmd = 'ANTS 3 -m MI[{},{},1,128] -i 0 -o {} --rigid-affine true' \
...
for mov_T2_name in mov_T2_names:
    ...
    cmd = 'ANTS 3 -m CC[{},{},1,2] -i 0 -o {} --rigid-affine true' \
        .format(os.path.join(path_img_T2, fix_T2_name), mov_T2_name, out_T2_name)
    os.system(cmd)


path_seg_T1 = config['preprocess']['resample']['source']
...
for mov_T1_name in mov_T1_names:
    ...
    seg = sitk.ReadImage(os.path.join(path_seg_T1, out_file_name.split('ceT1.nii.gz')[0] + 'Label.nii.gz'))
